...
function ensureMatchesColumnsAndDropdowns() {
  ...
  // Cambio 21: Desplegable Sí/No en RESERVA
  var reservaColDrop = headers["RESERVA"] || 11;
  if (reservaColDrop && maxRows > 1) {
    var reservaRule = SpreadsheetApp.newDataValidation()
      .requireValueInList(["Sí", "No"], true)
      .setAllowInvalid(true)
      .build();
    safeSetDataValidation(sheet.getRange(2, reservaColDrop, maxRows - 1, 1), reservaRule);
  }
...
function getEstadosPorEtapa() {
  ...
  // Cambio 21/22: Asegurar estado 'cita reservada' y su color en Servicio al Cliente
  if (!result.COLOR_MAP["CITA RESERVADA"]) {
    result.COLOR_MAP["CITA RESERVADA"] = "#B4A7D6";
  }
  var hasCitaReservada = false;
...
/**
 * Formatea una fecha en texto legible en español (ej. "25 de noviembre, 2026").
 */
function formatFechaEspanolSupervision(val) {
  if (!val) return "-";
  var dt = parseFechaSupervision(val);
...
function generarPanelSupervisionMaria(customDesde, customHasta, isAutomatedRun) {
  ...
  // 1. Fila 1 (banner superior antiguo) eliminada por completo.

  // 2. Fecha de actualización automática (Fila 2 - alineada a la izquierda, sin fondo, gris claro)
  ...
  sheet.getRange("F3:H3").merge()
    .setValue("🔄 Recalcular con Filtro")
    .setFontWeight("bold")
    .setFontColor("#961500")
    .setHorizontalAlignment("left")
  ...
  // 4 & 5. Tarjetas KPI viejas (filas 4-5) y banners de tiempos promedio (filas 6-7) ELIMINADOS.
  // Fila 4 queda como espacio visual limpio entre el filtro y la tabla.
  ...
  for (var h = 0; h < masterHeaders.length; h++) {
    sheet.getRange(7, h + 1).setValue(masterHeaders[h]).setFontWeight("bold").setBackground("#E8EAED").setHorizontalAlignment("center").setWrap(true);
  }
  sheet.setRowHeight(7, 42); // más alta para acomodar 2 líneas de texto
  ...
  for (var p = 0; p < psycList.length; p++) {
    ...
    var normPsycUpper = (pName || "").toString().trim().toUpperCase();
    if (excludedFromPanel[normPsycUpper]) continue; // Excluir SOFI, ALEJA, MAPE D, MANU y roles de dirección
    ...
    psychologistsData.push({
      ...
      fechaEnBlanco: formatFechaEspanolSupervision(oldestPendingDate),
      estado: estadoPsic
    });
  }

  // Ordenar psicólogas por matches aprobados (mayor a menor, desempate por total slots)
  ...
  // Totales acumulados (se preservan intactos para alimentar con exactitud las Tablas 2 a 5)
  ...
  // Escribir datos de la Tabla Rendimiento Matchmaker (12 columnas agrupadas - Cambio 30)
  for (var rIdx = 0; rIdx < psychologistsData.length; rIdx++) {
    var pData = psychologistsData[rIdx];
    var rowNum = 8 + rIdx; // Cambio 30: la fila de banner de grupo corrió los encabezados a la fila 7 y los datos a la 8
    ...
    sumGap += pData.gap;
    sumNoAprobados += pData.noAprobados;
    sumTroubleOnly += pData.troubleOnly;
  ...
  // Fila TOTAL EQUIPO: ELIMINADA según feedback de Lina (la tabla pasa a ser de vista individual únicamente).
  var lastTable1Row = 7 + psychologistsData.length; // Cambio 30: encabezados ahora en fila 7 (antes 6), por la fila de banner de grupo

  // ─── 6. TABLA 2: EMBUDO DE CONVERSIÓN END-TO-END (6 ETAPAS) ─────────────────
  var startRowT2 = lastTable1Row + 2;
  ...
  // 12. Actualizar gráfico de barras posicionado al lado de la tabla
  try {
    agregarGraficoAprobadosPorPsicologa();
  } catch (chartErr) {
    Logger.log("Aviso al actualizar gráfico de barras: " + chartErr.message);
  }

  Logger.log("✅ Pestaña privada '🔒 SUPERVISIÓN MARÍA' generada con panel Rendimiento Matchmaker.");
  ss.toast("Panel Rendimiento Matchmaker generado y actualizado.", "Panel Listo", 5);
}
...
function handleMatchesEdit(sheet, row, col, newValue, oldValue) {
  ...
  var reservaCol = headers["RESERVA"] || 11;
  if (reservaCol && col === reservaCol) {
    var reservaVal = (newValue || sheet.getRange(row, reservaCol).getValue() || "").toString().trim().toUpperCase();
    if (reservaVal === "SÍ" || reservaVal === "SI") {
      sheet.getRange(row, matchCol).setValue("cita reservada").setBackground("#B4A7D6");
      SpreadsheetApp.getActiveSpreadsheet().toast("📌 Reserva confirmada. Estado Total actualizado a 'cita reservada'.", "Reserva Confirmada", 5);
    }
  }
}
...
function returnCandidatesToPsychologists(matchesSheet, row, cellA, cellB, rejectionReason) {
  ...
  var countRetornadas = 0;
  var hasPersonB = (cellB && cellB.text && cellB.text.toLowerCase() !== "por definir");
  var totalEsperadas = hasPersonB ? 2 : 1;
  ...
  if (hasPersonB) {
    ...
    sheetPsycB = psycB ? findPsychologistSheet(psycB) : null;
  ...
  // Cambio 24: Toast verídico basado en el resultado real de la operación de retorno
  if (missingPsyc) {
    if (countRetornadas === 1) {
      var personaRetornada = sheetPsycA ? cellA.text : (cellB ? cellB.text : "");
      var personaPendiente = !sheetPsycA ? cellA.text : (cellB ? cellB.text : "");
      SpreadsheetApp.getActiveSpreadsheet().toast("⚠️ Retorno parcial: solo '" + personaRetornada + "' fue retornada. '" + personaPendiente + "' no tiene psicóloga asignada (marcada en amarillo).", "Retorno Parcial", 7);
    } else {
      SpreadsheetApp.getActiveSpreadsheet().toast("⚠️ Match rechazado: ninguna persona fue retornada porque no tienen psicóloga asignada. Celdas marcadas en amarillo.", "Revisión Requerida", 7);
    }
  } else {
    ...
    var toastSuccess = totalEsperadas === 2 
      ? "Match cerrado. Ambas personas retornadas a sus psicólogas." 
 ...
 * al lado de la Tabla Rendimiento Matchmaker del Panel de Supervisión MPS.
 */
function agregarGraficoAprobadosPorPsicologa() {
  ...
  // Buscar dinámicamente la fila de inicio de datos (debajo del encabezado 'Psicóloga')
  var headerRow = -1;
  for (var r = 1; r <= 15; r++) {
    var val = (sheet.getRange(r, 1).getValue() || "").toString().trim().toLowerCase();
    if (val === "psicóloga" || val === "psicologa") {
      headerRow = r;
      break;
    }
  }
  ...
  while (sheet.getRange(lastDataRow, 1).getValue() !== "" && sheet.getRange(lastDataRow, 1).getValue() !== "TOTAL EQUIPO") {
    lastDataRow++;
  }
  lastDataRow--;
  if (lastDataRow < firstDataRow) return;

  var numRows = lastDataRow - firstDataRow + 1;
